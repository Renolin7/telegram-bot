...
def nextnow(ch_id,m_id,data):
  ...
  if data=='2':
    text=check()
    if 'no class scheduled' in text.lower() or 'holiday' in text.lower() or 'link not updated' in text.lower() or 'break' in text.lower():
      key=json.dumps({'inline_keyboard':[[{'text':'Go back','callback_data':'0'}]]})
    ...
    text=nextcl()
    if 'no classes scheduled' in text.lower() or 'holiday' in text.lower() or 'link not updated' in text.lower() or 'break' in text.lower():
      key=json.dumps({'inline_keyboard':[[{'text':'Go back','callback_data':'0'}]]})
    elif 'theory of computation' in text.lower():
      key=json.dumps({'inline_keyboard':[[{'text':'Join class','url':toc()}],[{'text':'Go back','callback_data':'0'}]]})
    elif 'web technology' in text.lower():
